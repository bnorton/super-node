module SuperNode
  class FacebookNode
    ...
    def paginate!(paging)
      ...
      @pagination ||= {
        'next_page' => {
          ...
          'relative_url' => SuperNode::Facebook.url_from_paging(paging['next']),
          'method' => 'GET',
        },
        'previous_page' => {
          'access_token' => access_token,
          'relative_url' => SuperNode::Facebook.url_from_paging(paging['previous']),
          'method' => 'GET',
        }
      }
    end
    ...
    def next_page
      @next_page ||= SuperNode::FacebookNode.new(pagination['next_page'])
    end

    def previous_page
      @previous_page ||= SuperNode::FacebookNode.new(pagination['previous_page'])
    end
